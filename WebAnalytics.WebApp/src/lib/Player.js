...
class Player {
    ...
    play(data, {onPlaying, onEnd}) {
        ...
        this._onEnd = onEnd;
        this._loop = new Loop(30, this._renderFrame.bind(this));
        this._playing = true;
        this._loop.start();
    }
    ...
    _renderFrame() {
        ...
        if (this._fragmentIndex > this.data.length - 1) {
            this.stop()
            if (this._onEnd) this._onEnd()
            return
        }

        let currentFragment = this.data[this._fragmentIndex];

        if (this._dataFrameIndex > currentFragment.frames.frames.length - 1) {
            this._fragmentIndex++;
            this._dataFrameIndex = 0;
            return;
        }
